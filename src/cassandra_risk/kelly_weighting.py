...
def kelly_weighted_probability(becker_probability: float) -> float:
    probability = clamp(float(becker_probability), 0.0, 1.0)
    return probability * kelly_fraction(probability)
...
def apply_kelly_weighting(
    ...
    for day_string, events in daily_events.items():
        ...
        for event_id, row in events.items():
            ...
            fraction = kelly_fraction(becker_probability)
            weighted_probability = kelly_weighted_probability(becker_probability)
